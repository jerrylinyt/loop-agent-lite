...
- 實跑每條可在 child 重現的 DoD + validate；明確只能在 integration worktree 執行的 DoD
  由 parent CAS 後負責。首次 pre-CAS confirm 尚無 integration-only 結果是預期時序，Agent
  不得因此報 issue 或拒絕 done，也不得自行宣稱該 DoD 已 PASS。
- 發現缺陷就修、commit、停止；下一輪再獨立確認。
- child 可重現的 DoD 與 validate 全部通過且無異動才 `done merge-main`；integration-only gate
  由 parent CAS 後執行。
...
6. 產生隔離 Dashboard config，只複製 label=`codex` 的命令；workspace/repo roots 指向 temp。
   ...
   四小時整體 gate 約束。
   DR-1 使用 shipped flag threshold 10，且 Agent 可以在收斂途中合理補強 plan、讓 flag 歸零重算；
   「等待核准」因此是獨立的最長 2 小時 bounded planning wait，不得沿用一般 30 分鐘 child wait。
   等待期間仍同步監看 terminal Fleet failure，且受同一次 Playwright 4 小時總上限約束。
