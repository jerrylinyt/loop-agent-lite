...
finalizing（terminal_intent=completed、owner硬中斷）──Resume recovery──→ finalizing
```
...
| cancel_requested/finalizing_cancel、owner仍存活 | idempotent且繼續取消 | conflict | idempotent |
| cancel_requested/finalizing_cancel、owner已消失 | 新recovery owner繼續取消 | 新recovery owner繼續取消 | idempotent但不作terminal projection repair |
| finalizing | conflict；不打斷durable finalization | conflict | conflict；完成或以terminal_intent=completed轉blocked待重播 |
| finalizing、terminal_intent=completed、owner已消失 | conflict | 新recovery owner重播finalizing | conflict |
...
- operator修復後若worktree仍在，cleanup retry以新observation token顯式supersede舊pre-intent blocked
  REMOVE；若operator已完整移除path、registry與task ref，也要以fresh fully-absent observation寫入
  `already-removed` intent/receipt/result，不能只把aggregate標cleaned卻留下blocked global lease。
...
1. Dashboard launcher先經central helper取得primary Git run lock、audit parallel operation lease並以
   ...
   在同一fence內擷取clean primary的完整branch ref與HEAD SHA，再terminalize marker。不同Dashboard
   process不共用可覆寫的pending path；audit失敗時零primary/base state
...
3. supervisor仍在base lock內claim`parallel-launcher` RepoOwnerFence、重驗primary clean/non-detached，
   並把branch/HEAD與Dashboard argv攜帶的expected pair逐字比對；任一改變都在immutable run artifact
   發布前fail-closed，封住Dashboard釋放marker到supervisor接手間的handoff空窗；
...
6. 執行`INITIALIZE_RUN_REFS`，以expected-absent CAS建立canonical safe sync ref指向start SHA並寫
   ...
   PREFLIGHT與INITIALIZE。canonical PREFLIGHT若因validator失敗成為terminal blocked，Resume只可在fence
   舊child後byte-for-byte重播；durable Abort claim則不得要求validator恢復，而是在完整pristine audit後
   先寫immutable `startup/pristine-abort-init.json`，再只執行mechanical INITIALIZE，讓取消、SHUTDOWN與
   terminal projection repair仍有canonical sync ref。marker綁舊blocked lease hash、兩個request hash與
   claimed Abort identity；INIT crash只能憑此marker重播，不能接受無PREFLIGHT proof的任意INIT。
   部分或非canonical journal一律blocked；receipt落盤後才可派工。
...
quiesce為blocked。`terminal_status=blocked`同樣沒有immutable result，普通owner不可接手；Parallel
recovery只有在request/spec/op-id完全一致、current reaped child identity也已exact fence後，才可CAS重播。
不同operation/request預設一律`LeaseBusy`，即使executor帶通用recovery callback也不能覆蓋blocked
lease。跨request取代只能走顯式byte-CAS supersession：pristine Abort的PREFLIGHT→INITIALIZE必須綁
immutable marker內的舊lease hash/nonce與前後request hash；cleanup retry則只接受同task/outcome、已audit
且完全沒有remove intent/receipt的blocked REMOVE。authorizer在短lock外完成fence，回鎖後逐byte比較舊
lease才可發布successor，其他caller不能借一般`execute`或exact-replay API繞過。
若immutable operation result已落盤、但process在lease terminalization前死亡或terminal write暫時失敗，
cached-result fast path不得直接回成功；它必須先以同一request的fence/byte-CAS收斂exact pending/blocked
lease，再用既有result terminalize且不得重跑operation。若目前lease無法取得recovery authority就維持
`LeaseBusy`，不能留下「caller看見成功、全域lease仍blocked/nonterminal」的分裂狀態。
...
5. terminal lease只能由持operation-fence lock者，在guardian/payload group確定消失且journal outcome
   ...
   executor或recovery owner完成terminal transition。guardian EOF與force fence的wait都有上限；超時時
   保留ACKed identity、containment handle與active blocked projection，不能無限持base/global lock，也不能
   偽造reaped後繼續cleanup；fresh-start與recovery兩條入口都要收斂為durable blocked/exit code 2，不把
   第二次quiesce失敗洩漏成未分類CLI例外。
...
Parallel Loop runner；必須有合法 plan import 且 `start_phase=exec`。completed/cancelled且owner已停止時
另顯示終態Delete，仍走既有no-follow workspace刪除guard，不碰target repo或已整合commits。

transition狀態的控制仍須保留無CLI recovery路徑：`pause_requested`可顯示「重試 Pause」；
`cancel_requested/finalizing_cancel`在owner不再running時可顯示單一「重試取消清理」並走
Resume recovery，以同一路徑同時涵蓋cleanup重播與aggregate已terminal、base projection落後的修復；
`finalizing + terminal_intent=completed`只有在Dashboard投影確認owner已不再running時，才顯示
「重試完成收尾」並走Resume recovery。這些按鈕不創造新狀態轉移，只重播既有durable intent。
