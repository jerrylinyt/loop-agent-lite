...
- 普通 Workspace 必須停止。
- Parallel base 必須已停止且 durable status 是終態 `completed` 或 `cancelled`。`paused`／`blocked`／`finalizing*` 都不可刪；先 Resume 完成 recovery，或 Abort 並完成取消清理。
- Managed worker 不可直接刪除；由 parent supervisor 在 child 已退出、lock 已釋放且 worktree 安全後清理／歸檔。
...
- Parallel 的 frozen plan、run report，以及需要保留的 `parallel/<run-id>/` durable audit artifacts。

Target repo Git commit 本身不會因刪 workspace 消失，但 workspace 專屬的 coordinator 關聯與 log 會消失。

## 操作步驟

1. 普通 Loop 用「本輪後停止」並等到 idle；Parallel 則確認已到 `completed`／`cancelled`，不是只有 supervisor process 停止。
2. 在詳細頁按紅色「刪除」。
3. 逐列閱讀確認視窗。
...
2. 包含內容符合預期：state、history、console、logs、prompts、snapshots、REPORT；Parallel base 還包含 run 目錄與 durable artifacts。
...
- Parallel 已整合進 primary 的 commits 不受影響；刪除 base 不是 Git rollback。終態前由 supervisor 管理的 worker cleanup 必須已先收斂。
...
| 只想暫停 Parallel | Pause；不要刪除 |
| Parallel blocked 或 cleanup 未完成 | 依 error Resume recovery，或 Abort 到 `cancelled`；不可直接刪 base／worker |
| 想刪 target repo | Dashboard workspace 刪除不會做；走獨立 repo 管理流程 |

## 完成檢查

- [ ] Workspace 已停止。
- [ ] 若為 Parallel，status 已是 `completed`／`cancelled`，且沒有 live supervisor/control job。
