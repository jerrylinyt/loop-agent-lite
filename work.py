...
def current_dispatch(ws):
    ...
    try:
        dispatch = json.loads((ws / "dispatch.json").read_text(encoding="utf-8"))
...
def cmd_create_plan(ws, argv):
    ...
    signal_path(ws, "called_create_plan", token).write_text("", encoding="utf-8")
...
def cmd_plan_ok(ws, argv):
    ...
    signal_path(ws, "signal_plan_ok", token).write_text("", encoding="utf-8")
...
def cmd_done(ws, argv):
    ...
    signal_path(ws, "signal_done", token).write_text("", encoding="utf-8")
...
def cmd_issue(ws, argv):
    ...
    with open(ws / f"pending_issues.{token}", "a", encoding="utf-8") as f:
