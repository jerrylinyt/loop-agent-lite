...
export default function NotifyModal({ config, beginOperation, endOperation, onClose, onSaved }: {
  ...
  const busyPending = useRef(false);

  const test = async () => {
    if (busyPending.current) return;
    const token = beginOperation("launcher:notify-test");
    if (!token) return;
    busyPending.current = true;
    setBusy(true);
    setMessage("測試中…");
    setTestOutput("");
    try {
  ...
  const save = async () => {
    if (busyPending.current) return;
    const token = beginOperation("launcher:notify-save");
    if (!token) return;
    busyPending.current = true;
    setBusy(true);
    setMessage("儲存中…");
  ...
  const requestClose = () => { if (!busyPending.current) onClose(); };

  return (
    <Modal title="終態通知管理" description={`loop 完成／停機時執行的命令；只寫個人設定：${config.personal_config_path ?? "dashboard.config.local.json"}`} closeDisabled={busy} onClose={requestClose} footer={
      <><button type="button" className="secondary-button" disabled={busy} onClick={requestClose}>取消</button><button type="button" className="primary-button" disabled={busy} onClick={() => void save()}>儲存通知設定</button><span role="status">{message}</span></>
    }>
      <fieldset className="launcher-fieldset" disabled={busy}>
