...
import type { ConfigResponse, DashboardConfig, LaunchRunner, StartupResponse, WorkspaceState, WorkspaceSummary } from "../../shared/api/types";
...
function ExecutionSettingsFields({ value, onChange, parallel = false }: {
  ...
  return (
    <div className="number-grid">
      ...
      {parallel && <label>最大並行 workers<input type="number" min={1} value={value.maxParallel} onChange={(event) => onChange({ maxParallel: +event.target.value })} /></label>}
      {parallel && <label>Worker 重啟上限<input type="number" min={1} value={value.workerRestartLimit} onChange={(event) => onChange({ workerRestartLimit: +event.target.value })} /></label>}
    </div>
  );
}
...
export default function LauncherModal({
  ...
  // runner 模式切換：loop/parallel 共用核心欄位；ralph 走 RalphLaunchForm，footer 以 signal 驅動送出。
  const [runnerMode, setRunnerMode] = useState<LaunchRunner>("loop");
  ...
  const launchGuard = useStaleGuard();

  const repo = repoChoice === "__custom__" ? customRepo.trim() : repoChoice;
  const parallelMode = runnerMode === "parallel";
  ...
  useEffect(() => () => launchGuard.cancelPending(), [launchGuard]);
  ...
  useEffect(() => {
    void getJson<ConfigResponse>("/api/config").then((response) => {
      ...
      setSettings({
        ...
        maxParallel: response.defaults.max_parallel ?? 2,
        workerRestartLimit: response.defaults.worker_restart_limit ?? 3,
      });
    });
  }, [templateConfig]);
  ...
  useEffect(() => {
    ...
    setSettings((value) => ({
      ...
      maxParallel: templateConfig.max_parallel ?? value.maxParallel,
      workerRestartLimit: templateConfig.worker_restart_limit ?? value.workerRestartLimit,
    }));
  }, [config, templateConfig]);

  useEffect(() => {
    // A repo change starts a fresh hydration epoch.  Once the user edits the
    // name, an in-flight /api/state response must not overwrite that input.
    ...
    void (async () => {
      if (matchingWorkspace) {
        ...
        if (existing && !existing.error) {
          ...
          if (!nameEdited.current) setName(matchingWorkspace.name);
          setSettings({
            ...
            maxParallel: saved.max_parallel ?? config.defaults.max_parallel ?? 2,
            workerRestartLimit: saved.worker_restart_limit ?? config.defaults.worker_restart_limit ?? 3,
          });
          return;
        }
      }
  ...
  const launch = async () => {
    ...
    const planError = validatePlan(planJson, { required: parallelMode, allowStack: parallelMode });
    if (planError) return setMessage(`錯誤：plan.json 格式不對：${planError}`);
    const isCurrent = launchGuard.begin();
    setLaunching(true);
    setMessage("啟動中…");
    const body: Record<string, unknown> = {
      ...
      worker_restart_limit: settings.workerRestartLimit,
      pause_after_plan: parallelMode ? false : settings.pauseAfterPlan,
      reset_state: parallelMode ? false : resetState,
      new_branch: parallelMode ? false : newBranch,
      plan_json: planJson,
      start_phase: parallelMode ? "exec" : startPhase
    };
    if (parallelMode) body.runner = "parallel";
    if (goalFile && !parallelMode) {
      body.goal_content = await goalFile.text();
      if (!isCurrent()) return;
    }
  ...
  const hasPendingLaunchMutation = !!planJson.trim()
    || (!parallelMode && (!!goalFile || resetState || newBranch));
  ...
  const planError = validatePlan(planJson, { required: parallelMode, allowStack: parallelMode });
  const effectiveStartPhase = parallelMode ? "exec" : startPhase;
  const launchChanges = [
    { label: "goal.md", before: repoStatus?.goal === "committed" ? "沿用已 commit 版本" : repoMark(repoStatus?.goal), after: !parallelMode && goalFile ? `以 ${goalFile.name} 取代（${goalFile.size} bytes）` : "不變" },
    { label: "plan / phase", before: matchingWorkspace ? `${matchingWorkspace.plan_len ?? 0} tasks · ${matchingWorkspace.phase ?? "—"}` : "新 workspace", after: planJson.trim() ? `${importedPlanCount} tasks · ${effectiveStartPhase}` : resetState ? "重建 state" : "沿用" },
    ...
    { label: "收斂 / timeout", before: matchingWorkspace ? "現有 workspace 設定" : "預設設定", after: `flag>${settings.flagThreshold} · done≥${settings.doneThreshold} · round ${settings.roundTimeout}m · backoff ${settings.agentBackoffMax}s${parallelMode ? ` · workers≤${settings.maxParallel}` : settings.pauseAfterPlan ? " · 規劃後暫停" : ""}` },
    { label: "Git branch", before: repoStatus?.branch || "detached / unknown", after: !parallelMode && newBranch ? `建立 loop/${name.trim() || repo.split("/").filter(Boolean).slice(-1)[0] || "workspace"}` : "不切換" },
  ];
  const closeBlocked = launching || ralphStatus.launching;
  const closeLauncher = () => {
    if (closeBlocked) return;
    launchGuard.cancelPending();
    onClose();
  };
  const footer = tab !== "launch"
    ? <button type="button" className="secondary-button" onClick={closeLauncher}>關閉</button>
    : runnerMode === "ralph" ? (
      <>
        <button type="button" className="secondary-button" onClick={closeLauncher} disabled={closeBlocked}>取消</button>
      ...
      <>
        <button type="button" className="secondary-button" onClick={closeLauncher} disabled={closeBlocked}>取消</button>
        <button type="button" className="primary-button" onClick={launch} disabled={launching || !repo || (parallelMode && !!planError)}>啟動</button>
        <span className="inline-message" role="status">{message}</span>
      </>
    );

  return (
    <Modal title="啟動與管理" description="建立新 loop，或查看由這個 dashboard 啟動的工作" onClose={closeLauncher} closeDisabled={closeBlocked} wide footer={footer}>
      ...
      {tab === "launch" ? (
        <>
          <div className="segmented-tabs runner-mode-tabs" role="tablist" aria-label="Runner 模式">
            ...
            <button type="button" role="tab" aria-selected={runnerMode === "parallel"} className={runnerMode === "parallel" ? "active" : ""} onClick={() => setRunnerMode("parallel")}>Parallel Loop</button>
        ...
        <div className="form-grid launcher-form">
          {parallelMode && <div className="repo-status">Parallel Loop 只接受已凍結的匯入計畫，並固定從 exec 開始；相同 stack 的 task 必須連續。</div>}
          ...
          <div className="form-field">
            ...
            <input id="goal-file" type="file" accept=".md,.markdown,.txt" disabled={parallelMode} onChange={(event) => setGoalFile(event.target.files?.[0] ?? null)} />
            {parallelMode && <span className="label-help">Parallel 使用目前 branch 上已 commit 的 goal.md。</span>}
          </div>
          <PlanImportField value={planJson} onChange={setPlanJson} startPhase={startPhase} onStartPhaseChange={setStartPhase} onOpenPromptTemplate={() => setPromptTemplateMode("plan")} promptTemplateAvailable={promptTemplateAvailable} parallel={parallelMode} />
          {promptTemplateUnavailableReason && <p className="field-error" role="alert">Prompt 模板停用：{promptTemplateUnavailableReason}</p>}
          <label>Workspace 名稱 <span className="label-help">留空＝repo 目錄名</span><input value={name} onChange={(event) => { nameEdited.current = true; setName(event.target.value); }} /></label>
          ...
          <details className="advanced-settings">
            ...
            <ExecutionSettingsFields value={settings} parallel={parallelMode}
              onChange={(patch) => setSettings((value) => ({ ...value, ...patch }))} />
            <label className="checkbox-row"><input type="checkbox" checked={parallelMode ? false : settings.pauseAfterPlan} disabled={parallelMode} onChange={(event) => setSettings((value) => ({ ...value, pauseAfterPlan: event.target.checked }))} />規劃收斂後暫停：不自動進入執行期，需回 Dashboard 按「運行」開始執行{parallelMode && <span className="label-help">（Parallel 固定從 exec 開始）</span>}</label>
            <label className="checkbox-row"><input type="checkbox" checked={parallelMode ? false : resetState} disabled={parallelMode} onChange={(event) => setResetState(event.target.checked)} />重置 workspace state（清除舊進度）</label>
            <label className="checkbox-row"><input type="checkbox" checked={parallelMode ? false : newBranch} disabled={parallelMode} onChange={(event) => setNewBranch(event.target.checked)} />在新 branch 跑（loop/&lt;workspace 名&gt;）</label>
