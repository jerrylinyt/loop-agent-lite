...
export default function CliManagerModal({
  ...
  return (
    <Modal title="Agent CLI 管理" description={`新增、修改、刪除與測試 CLI；只寫個人設定：${config.personal_config_path ?? config.config_path ?? "dashboard.config.local.json"}`} onClose={onClose} wide footer={
      <><button type="button" className="secondary-button" onClick={onClose}>取消</button><button type="button" className="primary-button" disabled={saving} onClick={() => void save()}>{saving ? "儲存中…" : "儲存 CLI 設定"}</button><span role="status">{message}</span></>
    }>
