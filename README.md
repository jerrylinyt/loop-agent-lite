...
- 執行期若在 Agent 輪途中被立即停止、repo 留有未完成變更且 workspace 仍有合法綠點，詳細頁會額外顯示「Resume」。此操作保留中斷現場並略過啟動時的 dirty-tree 阻擋與 Validate，直接交給下一輪 Agent；Git／單 writer lock、workspace identity、受保護檔案與綠點檢查仍保留，下一輪結束後也仍須照常 Validate。一般「運行」維持完整 preflight。
