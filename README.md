...
- 工具列「📺 總覽」切換電視牆模式：聚合統計（執行中／完成／任務完成率）＋全 fleet 即時卡片，點卡片切入；卡片與事件推播共用 SSE，不另開輪詢；可用名稱搜尋與「全部／需關注／執行中／已完成」篩選卡片；紅連跳、停滯、issues、Agent 異常、checkpoint 復原與 goal 變更都會在卡片上標示；搭配 `--read-only` 適合掛牆監控。
