...
TRACK_RE = re.compile(r"[a-z0-9][a-z0-9_-]{0,23}")
MAX_TRACKS = 8
EXAMPLE = ('[{"order": 1, "task": "描述（含 DoD）", "ref": "PLAN.md#段落", '
           '"track": "main", "scope": ["src/**"]}, '
           '{"order": 2, "task": "ref/scope 可省略", "track": "main"}]')
...
def validate_plan(plan):
    ...
    for i, t in enumerate(plan):
        if not isinstance(t, dict):
            ...
            continue
        extra = set(t) - {"order", "task", "ref", "track", "scope"}
        if extra:
            errs.append(f"第 {i} 項有未知欄位 {sorted(extra)},只允許 order/task/ref/track/scope")
        ...
        track = t.get("track")
        if not isinstance(track, str) or not (
                track == "@final" or TRACK_RE.fullmatch(track)):
            errs.append(
                f"第 {i} 項 track 必填且須為 {TRACK_RE.pattern}，或保留名 @final")
        if "scope" in t:
            scope = t["scope"]
            if (not isinstance(scope, list) or not scope or
                    any(not isinstance(item, str) or not item.strip() for item in scope)):
                errs.append(f"第 {i} 項 scope 必須是非空字串陣列(可省略)")
    ...
    tracks = {t.get("track") for t in plan if isinstance(t, dict) and isinstance(t.get("track"), str)}
    if len(tracks) > MAX_TRACKS:
        errs.append(f"track 總數不可超過 {MAX_TRACKS},收到:{len(tracks)}")
    if errs:
        return None, errs
    normalized = []
    for task in sorted(plan, key=lambda x: x["order"]):
        item = {"order": task["order"], "task": task["task"].strip(),
                "ref": (task.get("ref") or None), "track": task["track"]}
        if "scope" in task:
            item["scope"] = [value.strip() for value in task["scope"]]
...
def cmd_done(ws, argv):
    ...
    if dispatch.get("phase") not in {"exec", "merge"} or not cur_id:
        die("目前不在執行或整合收斂期，或無派發任務，done 不可用")
    if argv[0] != cur_id:
