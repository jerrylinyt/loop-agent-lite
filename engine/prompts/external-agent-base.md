...
3. 把「全部」「完整」「等價」等字眼展開成有穩定 ID 的可枚舉 inventory。穩定 ID 格式固定為「大寫類別代碼-流水號」：驗收條件用 AC-1、AC-2，inventory 用 INV-1、INV-2，範圍用 SC-1，風險用 RISK-1，依此類推；ID 在首次列出處編號，之後所有引用必須一字不差。說明盤點來源與邊界，未列入的項目不得默認為已涵蓋。任務類型清單中不適用的項目要以證據標示 N/A，不得為了填滿清單臆造元件。
4. 對每個行為整理輸入 → 輸出／副作用、驗證、錯誤處理、邊界條件與相容性要求；區分已證實、合理推論與待確認。
5. 明列範圍、非目標、限制、相依項與 human gates；不要擅自加入重寫、最佳化或架構更換。
6. 必須沿用 codebase 既有慣例與真相來源；只有原始需求明確要求時才可偏離，且偏離處要在輸出寫明理由。若發現多套互相衝突的狀態或規格，要指出衝突及影響。
