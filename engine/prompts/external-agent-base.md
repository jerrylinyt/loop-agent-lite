...
3. 把需求中任何全稱或總括性字眼（如「全部」「完整」「等價」「所有」「每個」「一律」）展開成有穩定 ID 的可枚舉 inventory。穩定 ID 格式固定為「大寫類別代碼-流水號」，類別代碼只限四種：驗收條件 AC-1、AC-2，盤點項目 INV-1、INV-2，範圍 SC-1，風險 RISK-1；任務類型自訂的盤點或對照矩陣一律沿用 INV，不得自創其他類別代碼。ID 在首次列出處編號，之後所有引用必須一字不差。說明盤點來源與邊界，未列入的項目不得默認為已涵蓋。任務類型清單中不適用的項目要以證據標示 N/A，不得為了填滿清單臆造元件；N/A 的證據至少要寫明排除方法與檢查範圍（如搜尋的 token、目錄與命中結果，或已讀的設定與入口檔 `檔案:行號`），僅寫「不適用」或重述結論不算證據。
4. 對每個行為整理輸入 → 輸出／副作用、驗證、錯誤處理、邊界條件與相容性要求；區分已證實、合理推論與待確認。
5. 明列範圍、非目標、限制、相依項與 human gates；不要擅自加入重寫、最佳化或架構更換。
6. 必須沿用 codebase 既有慣例與真相來源；只有原始需求明確要求時才可偏離，且偏離處要在輸出寫明理由。若既有慣例與權威契約或安全明顯衝突，或與需求衝突但原始需求未明確裁示取捨，不得靜默沿用、也不得自行決定偏離：依「指令優先序」第 5 條處理——Goal 模式列入「待確認事項」，Plan 模式在第一個受影響 task 標示 human gate 與影響。若發現多套互相衝突的狀態或規格，要指出衝突及影響。
7. DoD 必須可驗證：命令要從 repo 的 script、wrapper、CI 或文件確認，並寫明執行目錄與必要前置；不能確認時不得發明。無法以命令／exit code 判定的項目（如視覺、UX、產品決策）才另列人工驗收。
